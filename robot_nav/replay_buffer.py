...
class RolloutReplayBuffer(object):
    ...
    def sample_batch(self, batch_size):
        max_len = 5
        if self.count < batch_size:
            batch = random.sample(list(itertools.islice(self.buffer, 0, len(self.buffer)-1)), self.count)
        else:
            batch = random.sample(list(itertools.islice(self.buffer, 0, len(self.buffer)-1)), batch_size)
        ...
        r_batch = np.array([batch[i][idx[i]][2] for i in range(len(batch))]).reshape(-1, 1)
        t_batch = np.array([batch[i][idx[i]][3] for i in range(len(batch))]).reshape(-1, 1)
